...
bool TextureImportConfiguration::Config()
{
	...
	if (format == TF_TGA)
		if (ImGui::Checkbox("Create stamp", &tga_stamp))
			changed = true;
	if (format == TF_JPG)
		if(ImGui::SliderInt("Quality", &jpg_quality, 0, 99, "%i"))
			changed = true;
	if (format == TF_PNG)
...
bool TextureLoadConfiguration::Config()
{
	...
	if (mip_mapping)
	{
		...
		if (!max_anysotropy)
		{
			...
			ImGui::SameLine();
			if(ImGui::SliderFloat("", &anysotropy_level, 0.0f, max_possible_anystropy))
				changed = true;
		}
	}
	...
	if (ImGui::TreeNodeEx("Filters", ImGuiTreeNodeFlags_Framed))
	{
		...
		if (blur_gaussian || blur_average)
		{
			...
			if(ImGui::SliderInt("", &blur_iterations, 0, 10))
				changed = true;
			ImGui::Columns(2);
		}
		...
		if (contrast)
		{
			...
			if(ImGui::SliderFloat("", &amount_of_contrast, 0.0f, 1.7f))
				changed = true;
			ImGui::Columns(2);
		}
		...
		if (gamma_correction)
		{
			...
			if(ImGui::SliderFloat("", &gamma_correction_factor, 0.0f, 3.0f))
				changed = true;
			ImGui::Columns(2);
		}
		...
		if (noise)
		{
			...
			if(ImGui::SliderFloat("", &noise_tolerance, 0.0f, 1.0f))
				changed = true;
			ImGui::Columns(2);
		}
		...
		if (pixelize)
		{
			...
			if(ImGui::SliderInt("", &pixels_across, 1, 30))
				changed = true;
			ImGui::Columns(2);
		}
		...
		if (sharpen)
		{
			...
			if(ImGui::SliderFloat("", &sharpening_factor, 0.0f, 2.5f))
				changed = true;
			ImGui::Text("Sharpen Iterations:");
			if(ImGui::SliderInt("", &sharpen_iterations, 1, 10))
				changed = true;
			ImGui::Columns(2);
		}
	...
	{
		ImGui::Text("Width:");
		if(ImGui::InputInt("", &scale_width))
			changed = true;

		ImGui::Text("Height:");
		if(ImGui::InputInt("", &scale_height))
			changed = true;

		ImGui::Text("Depth:");
		if(ImGui::InputInt("", &scale_depth))
			changed = true;
