...
{
	ImGui::Text("Ticks per second: %f\nDuration: %i\nNum Channels: %i", ticks_per_sec, duration, channels.size());
	ImGui::Checkbox("Loop",&loop);

	return false;
}

void Animation::AnimationClip::GetJointPos(const std::string & joint_name, float3x4 & joint_transform, const float3x4 & inverse_bind_pos, double anim_time, bool interpolation)
{
	for (int i = 0; i < channels.size(); i++)
		if (channels[i]->joint_name == joint_name)
			channels[i]->GetTransform(anim_time, joint_transform, inverse_bind_pos, interpolation);
}
...
void Animation::GetJointPos(const std::string & joint_name, float3x4 & joint_transform, const float3x4 & inverse_bind_pos, double anim_time, bool interpolation)
{
	if (source != nullptr)
		source->GetJointPos(joint_name, joint_transform, inverse_bind_pos, anim_time, interpolation);
	else
		LOG("Source not loaded");
}
